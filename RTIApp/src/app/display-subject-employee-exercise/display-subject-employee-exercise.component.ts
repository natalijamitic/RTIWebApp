...
@Component({
  selector: 'display-subject-employee-exercise',
  templateUrl: './display-subject-employee-exercise.component.html',
  styleUrls: ['./display-subject-employee-exercise.component.scss']
})
export class DisplaySubjectEmployeeExerciseComponent implements OnInit {
  ...
  public initFiles(): void {
    this.files = [];
    for (let file of this.subject.exercises) {
  ...
  public deleteFile(file): void {
    this.subject.exercises = this.subject.exercises.filter((f: string) => f != file);
    this.initFiles();
    this.updateSubject();
  }
  ...
  public sortFiles(): void {
    ...
    for (let file of this.files) {
      this.subject.exercises.push(file.original);
    }
    this.updateSubject();
  }
  ...
  public filesUploaded($event): void {
    for (let file of $event.fileNames) {
      this.subject.exercises.push(file);
    }
    this.initFiles();
    this.updateSubject();
  }
